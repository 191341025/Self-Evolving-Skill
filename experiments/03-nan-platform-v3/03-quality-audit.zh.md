...
## R6：时间衰减定点验证（2026-03-15, t=1 天）

v3 R1-R5 在同一天完成（2026-03-14），时间衰减始终 t=0。R6 在一天后运行，验证此前不可观测的时间相关特性。

### R6-1：自然衰减观测

t=1 天时 scan 显示全部 8 条知识已开始自然衰减：

| 条目 | 类型 | α | β | C (t=1d) | C (t=0d) | 衰减驱动 |
|------|------|---|---|----------|----------|---------|
| schema_map:5 | schema | 4.0 | 0.0 | 0.999 | 1.000 | λ=0.003, 高 α 减缓 |
| schema_map:9 | schema | 3.0 | 0.0 | 0.999 | 1.000 | λ=0.003, α=3 保护 |
| schema_map:13 | schema | 0.0 | 0.0 | 0.997 | 1.000 | λ=0.003, 无反馈 |
| query_patterns:9 | tool_exp | 0.0 | 0.0 | 0.995 | 1.000 | λ=0.005 |
| query_patterns:13 | qry_pat | 1.0 | 0.0 | 0.993 | 1.000 | λ=0.015, α=1 部分保护 |
| business_rules:5 | biz_rule | 0.3 | 0.3 | 0.992 | 1.000 | λ=0.008, α≈β 抵消 |
| business_rules:9 | biz_rule | 0.0 | 0.0 | 0.992 | 1.000 | λ=0.008 |
| query_patterns:5 | qry_pat | 0.0 | 0.0 | 0.985 | 1.000 | λ=0.015, 衰减最快 |

**三个设计特性现已可观测：**

1. **类型特异 λ**：同为 α=0 — query_pattern (C=0.985) 衰减速度是 schema (C=0.997) 的 5 倍
2. **贝叶斯减缓**：同为 query_pattern — α=1.0 (C=0.993) vs α=0 (C=0.985)
3. **α/β 抵消**：business_rules:5 (α=0.3, β=0.3) 与 business_rules:9 (α=0, β=0) C 值相同——比率 (β+1)/(α+1) 均为 1.0

### R6-2：衰减后知识的可用性

使用 schema_map:5 (C=0.999, TRUST) 执行查询。协议行为："直接使用，不提及置信度"——正确。查询成功后记录硬信号反馈：

- **反馈前**：C=0.999, α=4.0, confirmed=2026-03-14 (1d)
- **反馈后**：C=1.000, α=5.0, confirmed=2026-03-15 (0d) — **confirmed_at 刷新重置了时钟**

这证明了"用进废退"原则：活跃使用的知识保持新鲜，不用的知识自然老化。

### R6-3：衰减投影

如果不再使用，每条知识到达 VERIFY (C=0.8) 和 REVALIDATE (C=0.5) 的预计天数：

| 条目 | 类型 | α | β | 贝叶斯因子 | 到 VERIFY 天数 | 到 REVALIDATE 天数 |
|------|------|---|---|-----------|---------------|------------------|
...
**核心洞察：正反馈是最强的保护。** schema_map:5 (α=5) 需 446 天才到 VERIFY——是 schema_map:13 (α=0, 74 天) 的 6 倍。每次成功使用大约将条目寿命翻倍。而 query_patterns:5（无反馈、高 λ）仅 15 天就需要重验——系统正确识别出它是最缺乏实战检验的条目。

### R6 总结

| 检查项 | 结果 |
|-------|------|
| t=1d 自然衰减可观测 | **是** — C 值范围 0.985-0.999 |
| 类型特异 λ 区分 | **已确认** — schema 衰减速度是 query_pattern 的 1/5 |
| 贝叶斯反馈减缓 | **已确认** — α=4 条目衰减速度是同类型 α=0 条目的 1/6 |
| confirmed_at 刷新 | **已确认** — C 恢复 1.000，年龄重置为 0d |
| 衰减投影合理 | **是** — 15 天（易变）到 446 天（充分验证的 schema） |

R6 填补了 v3 原始质量审计中识别出的时间衰减覆盖缺口。
